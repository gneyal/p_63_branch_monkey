/**
 * Cloudflare Pages Function - Demo API Handler
 * Serves static demo data for the web demo at pages.dev
 */

// Demo data inline (loaded at build time from static files)
const DEMO_DATA = {
  tasks: {
    "tasks": [
...
function readOnlyError() {
  return jsonResponse({ error: 'Demo mode - read only', detail: 'This is a demo. Install locally for full functionality.' }, 403);
}
...
export async function onRequest(context) {
  ...
  try {
    // Tasks
    if (path === 'tasks') {
      if (method === 'GET') return jsonResponse(DEMO_DATA.tasks);
    ...
    // Versions
    if (path === 'versions') {
      if (method === 'GET') return jsonResponse(DEMO_DATA.versions);
      return readOnlyError();
    }
    if (path === 'versions/reorder') {
      return readOnlyError();
    }
    ...
    // Commit tree
    if (path === 'commit-tree' || path.startsWith('commit-tree?')) {
      return jsonResponse(DEMO_DATA.commitTree);
    }

    // Repo info
    if (path === 'repo/info') {
      return jsonResponse(DEMO_DATA.repoInfo);
    }
    if (path === 'repo/set') {
      return jsonResponse({ success: true, ...DEMO_DATA.repoInfo });
    }
    if (path === 'repo/search') {
      return jsonResponse({ suggestions: [] });
    }
    if (path === 'repo/list-projects') {
      return jsonResponse({ projects: [] });
    }

    // Example project
    if (path === 'example-project') {
      return jsonResponse({ exists: true, has_data: true, path: '/demo/demo-project' });
    }

    // Branches
    if (path === 'branches') {
      return jsonResponse(DEMO_DATA.branches);
    }
    if (path === 'branch/create') {
      return readOnlyError();
    }

    // Working tree
    if (path === 'working-tree') {
      return jsonResponse(DEMO_DATA.workingTree);
    }

    // Remote status
    if (path === 'remote/status') {
      return jsonResponse(DEMO_DATA.remoteStatus);
    }

    // Experiments
    if (path === 'experiments') {
      return jsonResponse(DEMO_DATA.experiments);
    }
    ...
    // Prompt logs
    if (path === 'prompt-logs' || path.startsWith('prompt-logs?')) {
      if (method === 'GET') return jsonResponse(DEMO_DATA.promptLogs);
      return readOnlyError();
    }
    if (path === 'prompt-logs/stats') {
      return jsonResponse(DEMO_DATA.promptLogs.stats);
    }
    if (path.match(/^prompt-logs\/\d+$/)) {
      return readOnlyError();
    }

    // Context endpoints
    if (path === 'context/counts') {
      return jsonResponse({ codebase: 1, architecture: 1, prompts: 1 });
    }
    if (path.match(/^context\/history\/(.+)$/)) {
      const type = path.split('/')[2];
      return jsonResponse(DEMO_DATA.contextHistory[type] || { entries: [] });
    }
    if (path.match(/^context\/latest\/(.+)$/)) {
      const type = path.split('/')[2];
      const history = DEMO_DATA.contextHistory[type];
      if (history && history.entries.length > 0) {
        return jsonResponse({ entry: history.entries[0] });
      }
      return jsonResponse({ entry: null });
    }
    if (path.match(/^context\/prompt\/.+$/)) {
      return jsonResponse({ prompt: "Generate a summary of this codebase..." });
    }
    if (path.match(/^context\/save\/.+$/)) {
      return readOnlyError();
    }
    if (path.match(/^context\/entry\/\d+$/)) {
      if (method === 'GET') {
        // Return first entry as sample
        return jsonResponse({ entry: DEMO_DATA.contextHistory.codebase.entries[0] });
      }
      return readOnlyError();
    }

    // Notes
    if (path.match(/^notes\/[a-f0-9]+$/)) {
      if (method === 'GET') return jsonResponse({ notes: [] });
      return readOnlyError();
    }
    if (path.match(/^notes\/[a-f0-9]+\/\d+$/)) {
      return readOnlyError();
    }

    // Prompts (commit prompts)
    if (path.match(/^prompts\/[a-f0-9]+$/)) {
      if (method === 'GET') return jsonResponse({ prompt: null, sha: path.split('/')[1] });
      return readOnlyError();
    }
    if (path === 'prompts/all/list') {
